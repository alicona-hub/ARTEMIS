...
generateCohortStats <- function(connectionDetails, cdmSchema, con_df, stringDF){
  ...
  if (nrow(m_df) > 0) {
    ...
    if (ncol(m_df) >= 2) colnames(m_df)[1:2] <- c("variable", "value")
    m_df$Category <- "Untreated"
    m_df$Gender <- "M"
  } else {
    message("No untreated male patients found — skipping male stats.")
    m_df <- data.frame(variable = character(),
                       value = numeric(),
                       Category = character(),
                       Gender = character(),
  ...
  if (nrow(f_df) > 0) {
    if (ncol(f_df) >= 2) colnames(f_df)[1:2] <- c("variable", "value")
    f_df$Category <- "Untreated"
    f_df$Gender <- "F"
  } else {
    message("No untreated female patients found — skipping female stats.")
    f_df <- data.frame(variable = character(),
                       value = numeric(),
                       Category = character(),
                       Gender = character(),
                       stringsAsFactors = FALSE)
  ...
  if (nrow(t_df) > 0) {
    if (ncol(t_df) >= 2) colnames(t_df)[1:2] <- c("variable", "value")
    t_df$Category <- "Untreated"
    t_df$Gender <- "Total"
  } else {
    message("No untreated total group found — skipping total stats.")
    t_df <- data.frame(variable = character(),
                       value = numeric())
  }
  #m_df$Category <- "Untreated"
  #m_df$Gender <- "M"
  ...
  # Create ATC output (pivot long -> wide)
  atc_output <- as.data.frame(
    reshape2::acast(
      rbind(t_df, rbind(m_df, f_df)),
      variable ~ Gender + Category,
      value.var = "value"
    )
  )
  
  # Expected 6-column structure (even if some are missing)
  expected_cols <- c(
    "M_Untreated", "F_Untreated", "Total_Untreated",
    "M_Treated", "F_Treated", "Total_Treated"
  )
  
  # Add any missing columns with 0 values
  for (col in expected_cols) {
    if (!col %in% colnames(atc_output)) {
      atc_output[[col]] <- 0
    }
  }
  
  # Replace NAs with 0s in existing columns
  atc_output[expected_cols] <- lapply(atc_output[expected_cols], function(x) {
    x[is.na(x)] <- 0
    as.numeric(x)
  })
  
  # Reorder columns to the standard order
  atc_output <- atc_output[, expected_cols, drop = FALSE]
